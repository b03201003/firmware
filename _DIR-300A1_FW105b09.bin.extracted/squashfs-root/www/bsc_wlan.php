...
if ($ACTION_POST!="")
{
	...
	if($f_enable=="1")
	{
		...
		{
			anchor("/wireless/wep");
			if(query("length")	!= $f_wep_len)			{set("length",	$f_wep_len);		$db_dirty++; $wps_cfg++;}
			if(query("format")	!= $f_wep_format)		{set("format",	$f_wep_format);		$db_dirty++; $wps_cfg++;}
			if(query("defkey")	!= $f_wep_def_key)		{set("defkey",	$f_wep_def_key);	$db_dirty++; $wps_cfg++;}
			if(query("key:".$f_wep_def_key)	!= $f_wep)	{set("key:".$f_wep_def_key,	$f_wep);$db_dirty++; $wps_cfg++;}
		}
		
		if($f_authentication=="2" || $f_authentication=="4" || $f_authentication=="6")	// wpa series
		{
			anchor("/wireless/wpa/radius:1");
			if(query("host")		!= $f_radius_ip1)	{set("host",	$f_radius_ip1);		$db_dirty++; $wps_cfg++;}
			if(query("port")		!= $f_radius_port1)	{set("port",	$f_radius_port1);	$db_dirty++; $wps_cfg++;}
			if($f_radius_secret1	!= $def_password)
			{ if(query("secret")	!= $f_radius_secret1){set("secret",	$f_radius_secret1);	$db_dirty++; $wps_cfg++;}}
		}
		else if($f_authentication=="3" || $f_authentication=="5" || $f_authentication=="7")// wpa psk series
		{
			anchor("/wireless/wpa");
			if(query("format")!=$f_wpa_psk_type)		{set("format",	$f_wpa_psk_type);	$db_dirty++; $wps_cfg++;}
			if($f_wpa_psk!=$def_psk)
			{
				if(query("key") != $f_wpa_psk)			{set("key", 	$f_wpa_psk);		$db_dirty++; $wps_cfg++;}
			}
		}
		if ($support_wps==1)
		{
...
function check()
{
	...
	<?if($support_wps=="1")	{ echo "	f_final.f_wps_enable.value		=\"\";\n"; }?>
